...
(defun depth-first (board element_location)
  (let ((elmt (aref board (car element_location) (cadr element_location)))
        (row (car element_location))
        (col (car element_location)))
    (print elmt)
    (print row)
    (print col)
    (if (= elmt (expt (array-dimension board 0) 2))
    ...
    (let ((neighbors (grab-neighbors board element_location)))
      (let ((up (car neighbors))
        ...
        (if (not (null up))
          (progn
            (if (= up next)
              (return-from depth-first (depth-first board (list (- row 1) col))))))
        (print "making it here?")
        (if (not (null left))
          (progn
            (if (= left next)
              (return-from depth-first (depth-first board (list row (- col 1)))))))
        (if (not (null down))
          (progn
            (if (= down next)
              (return-from depth-first (depth-first board (list (+ row  1) col))))))
        ...
        (if (null up)
          (progn
            (setf board (car (insert-element-into-board
                               (list (- row 1) col next) board nil)))
             (setf move_on (depth-first board (list (- row 1) col)))
             (if (= move_on -1)
               (insert-null-element (list (- row 1) col next) board)
             (return-from depth-first board))))
         (if (null left)
           (progn
             (setf board (car (insert-element-into-board
                                (list row (- col 1) next) board nil)))
             (setf move_on (depth-first board (list row (- col 1))))
             (if (= move_on -1)
               (insert-null-element (list row (-col 1) next) board)
             (return-from depth-first board))))
         (if (null down)
           (progn
             (setf board (car (insert-element-into-board
             ...
             (setf move_on (depth-first board (list (+ row 1) col )))
             (if (= move_on -1)
               (insert-null-element (list (+ row 1) col nil) board)
             (return-from depth-first board))))
         (if (null right)
           (progn
             (setf board (car (insert-element-into-board
             ...
             (setf move_on (depth-first board (list row (+ col 1))))
             (if (= move_on -1)
               (insert-null-element (list row (+ col 1) nil) board)
             (return-from depth-first board))))
         (return-from depth-first -1)))))
...
(defun solve-game ()
  (let ((input (ask-for-file)))
    ...
    (terpri)
    (print-board board)
    (setf board (depth-first board (find-elmt board min_elmt)))
    ))
